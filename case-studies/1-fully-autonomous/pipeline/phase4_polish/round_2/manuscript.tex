...
Write $\kappa = a + b$ for the precision of $\mathrm{Beta}(a,b)$. The vague component has $\kappa_0 = 2$ and each informative component has $\kappa_k = \alpha_0 + \beta_0 + n_k s_k$. By the law of total variance, $\mathrm{Var}_{\piDAP}[\theta] = \sum_k w_k \sigma_k^2 + \sum_k w_k(\mu_k - \mu)^2 \geq \sum_k w_k \sigma_k^2$, where $\sigma_k^2 = \mu_k(1-\mu_k)/(\kappa_k + 1)$ is the $k$-th component variance. Because mixing can only increase variance relative to within-component variance, the moment-matched precision $\ESS(\piDAP) = \mu(1-\mu)/\mathrm{Var}_{\piDAP}[\theta] - 1$ cannot exceed the precision of the most concentrated component. In our parametrization, the maximum component ESS is $\max_k \kappa_k$, with the vague component contributing $\kappa_0 = 2$ at weight $w_0$, yielding the stated bound. The argument is heuristic at the step relating mixture-mean $\mu(1-\mu)$ to the component-mean terms $\mu_k(1-\mu_k)$; a fully rigorous proof would require bounding the ratio $\mu(1-\mu)/\sum_k w_k \mu_k(1-\mu_k)$, which depends on component-mean spread. Numerical verification across all 35 LOO-CV folds confirms the bound holds with substantial slack (observed maximum $\ESS = 10.2$; bound $= 204$).
\end{proof}
...
Under the Favorable scenario---where graph structure is genuinely informative---KG-DAP achieves MAE $= 0.1258$, matching its real-data LOO-CV performance (0.1257), with a 25.9\% improvement over rMAP (0.1697). The paired bootstrap 95\% confidence interval for the MAE difference (KG-DAP minus rMAP) is $[-0.0440, -0.0438]$, confirming statistical significance. This validates that the LOO-CV results are not artifacts of the particular data realization.
...
& \textbf{KG-DAP} & \textbf{0.1258 (0.0001)} & 5.44 & \multirow{2}{*}{$[-0.0440, -0.0438]$} \\
...
\midrule
\multirow{5}{*}{Adverse}
& KG-DAP           & 0.1692 (0.0001) & 4.25 & \multirow{2}{*}{$[0.0012, 0.0014]$} \\
...
\midrule
\multirow{5}{*}{Mixed}
& \textbf{KG-DAP} & \textbf{0.1560 (0.0001)} & 4.53 & \multirow{2}{*}{$[-0.0302, -0.0300]$} \\
