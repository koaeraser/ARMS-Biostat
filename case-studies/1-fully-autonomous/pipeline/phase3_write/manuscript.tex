...
\begin{enumerate}
    ...
    \item Simulation studies with known ground truth under favorable, adverse, and mixed scenarios (Section~\ref{sec:results}).
\end{enumerate}
...
\begin{table}[t]
\centering
\caption{Leave-one-out cross-validation results across 35 NDMM trial arms. KG-DAP achieves the lowest MAE and RMSE while maintaining well-calibrated coverage. Bold indicates best performance; the Pooled method has extreme concentration (ESS $= 9075$) leading to nearly zero coverage.}
\label{tab:loocv}
\begin{tabular}{lcccccc}
\toprule
Method & MAE & RMSE & Coverage & Width & Log Pred & ESS \\
\midrule
\textbf{KG-DAP}   & \textbf{0.1257} & \textbf{0.1559} & 0.943 & 0.788 & \textbf{$-$5.028} & 5.59 \\
rMAP               & 0.1697          & 0.2009          & 0.886 & 0.778 & $-$5.179          & 5.60 \\
Equal Weight       & 0.1666          & 0.2003          & 0.971 & 0.812 & $-$5.070          & 4.22 \\
...
Drug-Only Similarity          & 0.1348 & 0.914 & $+$7.3  \\
...
\item \textbf{Composite similarity outperforms any single component.} Drug-only ($+$7.3\%) and population-only ($+$15.5\%) similarities are both worse than the composite, validating the multi-dimensional approach.
...
\caption{Component contribution analysis showing MAE for each ablation variant. Both graph structure and power-based weights contribute substantially; removing either degrades performance by $>$30\%.}
\label{fig:ablation}
\end{figure}
...
For each scenario, $B = 200$ Monte Carlo replications generate synthetic data from the known true rates using the real sample sizes. Table~\ref{tab:simulation} reports the results. All Monte Carlo standard errors are below 0.02\% of the corresponding effect sizes.

Under the Favorable scenario---where graph structure is genuinely informative---KG-DAP achieves MAE $= 0.1258$, matching its real-data LOO-CV performance (0.1257), with a 25.8\% improvement over rMAP (0.1696). This confirms that the LOO-CV results are not artifacts of the particular data realization.

Under the Adverse scenario---where graph structure is uninformative because rates are randomly shuffled---KG-DAP's MAE (0.1691) is only slightly worse than rMAP's (0.1680), a 0.7\% relative degradation. This graceful degradation is a key safety property: when the knowledge graph is uninformative, KG-DAP does not significantly harm inference. The uniform prior achieves the best MAE (0.1608) in this scenario, as expected, since no borrowing method can improve upon no-borrowing when the graph provides no signal.

Under the Mixed scenario---where half the trials have accurate graph-predicted rates and half are perturbed---KG-DAP retains a substantial 16.1\% improvement over rMAP (0.1562 vs. 0.1861). This demonstrates robustness to partial graph inaccuracy: the correctly predicted arms contribute enough signal to overcome the noise from the perturbed arms.

\begin{table}[t]
\centering
\caption{Simulation study results ($B = 200$ replications). Monte Carlo standard errors are reported in parentheses. Under the Favorable scenario, KG-DAP's advantage is largest; under the Adverse scenario, KG-DAP gracefully degrades to near-uniform performance rather than harming inference.}
\label{tab:simulation}
\begin{tabular}{llcc}
\toprule
Scenario & Method & MAE (SE) & ESS \\
\midrule
\multirow{5}{*}{Favorable}
& \textbf{KG-DAP} & \textbf{0.1258 (0.0002)} & 5.40 \\
& rMAP             & 0.1696 (0.0001)           & 5.54 \\
& Equal Weight     & 0.1666 (0.0001)           & 4.09 \\
& Uniform          & 0.1608 ($< 0.0001$)       & 2.00 \\
& Pooled           & 0.1799 (0.0001)           & 9075 \\
\midrule
\multirow{5}{*}{Adverse}
& KG-DAP           & 0.1691 (0.0001) & 4.25 \\
& rMAP             & 0.1680 (0.0001) & 6.72 \\
& Equal Weight     & 0.1666 (0.0001) & 4.14 \\
& \textbf{Uniform} & \textbf{0.1608 ($< 0.0001$)} & 2.00 \\
& Pooled           & 0.1633 ($< 0.0001$) & 9075 \\
\midrule
\multirow{5}{*}{Mixed}
& \textbf{KG-DAP} & \textbf{0.1562 (0.0002)} & 4.53 \\
& rMAP             & 0.1861 (0.0001)           & 5.28 \\
& Equal Weight     & 0.1850 ($< 0.0001$)      & 3.50 \\
& Uniform          & 0.1819 ($< 0.0001$)      & 2.00 \\
& Pooled           & 0.1943 (0.0001)           & 9075 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection*{Table S1: Sensitivity to Power Parameter $\beta$}

\begin{table}[h]
\centering
\caption*{Table S1: Full sensitivity analysis for the power parameter $\beta$. The safe operating range $[8, 30]$ shows minimal MAE variation.}
