"""
Simulation Study v3 — Optimized for speed.
...
def main():
    ...
    for sname, true_rates in scenarios.items():
        ...
        for b in range(B):
            rng = np.random.default_rng(42 + b * 997 + hash(sname) % 9973)
    ...
    df.to_csv(os.path.join(DATA_DIR, "simulation_summary.csv"), index=False)
    ...
    print(f"\nSaved: {os.path.join(DATA_DIR, 'simulation_summary.csv')}")
